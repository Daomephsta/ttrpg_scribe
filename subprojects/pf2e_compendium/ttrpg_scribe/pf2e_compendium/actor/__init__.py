...
class ActionsContainer(Iterable[Action]):
    def __init__(self, actions: Iterable[Action] = []) -> None:
        ...
        self._names_by_category: dict[Action.Category, list[str]] =\
            {category: [] for category in Action.CATEGORIES}
        for action in actions:
            self.add(action)

    def add(self, action: Action):
        self._by_name[action.name] = action
        self._names_by_category[action.category].append(action.name)

    def remove(self, name: str):
        action = self._by_name.pop(name)
        self._names_by_category[action.category].remove(name)
    ...
    def by_category(self, category: Action.Category) -> Iterable[Action]:
        names = self._names_by_category[category]
